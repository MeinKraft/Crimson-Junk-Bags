...
public class JunkBagEpicItem extends Item {
    public JunkBagEpicItem() {
    ...
    public void addInformation(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        if (CrimsonJunkBags.JunkBagEpicItemLoot.size() == 0)
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        return new ActionResult<>(CrimsonJunkBags.COMMON.onItemRightClick(worldIn, playerIn, handIn, CrimsonJunkBags.JunkBagEpicItemLoot), playerIn.getHeldItem(handIn));
    }
}